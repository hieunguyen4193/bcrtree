...
for (mouse.id in unique(mid.metadata$mouse)){
  ...
  for (input.yfp.case in names(yfp.mids)){
    path.to.05.output <- file.path(path.to.main.output, "05_output", sprintf("CDR3_%s", thres), mouse.id, sprintf("%s_YFP_MIDs", input.yfp.case))
    ...
    selected.mids <- yfp.mids[[input.yfp.case]]
    
    cloneset_files <- all.clones.tables[selected.mids]
    ...
    writexl::write_xlsx(clonesets, file.path(path.to.05.output, sprintf("clonesets_%s.xlsx", mouse.id)))
    
    #####---------------------------------------------------------------------#####
    ##### Group AA sequences to group based on 85% similarity
    #####---------------------------------------------------------------------#####    
    clonesetsdf <- readxl::read_excel(file.path(path.to.05.output, sprintf("clonesets_%s.xlsx", mouse.id)))
    ...
    writexl::write_xlsx(new.clonesetsdf, file.path(path.to.05.output, sprintf("clonesets_%s.split_clones_%s.xlsx", mouse.id, thres)))
    ...
    #####---------------------------------------------------------------------#####
    if (file.exists(file.path(path.to.05.output, sprintf("clones_%s.xlsx", mouse.id))) == FALSE){
      ...
      writexl::write_xlsx(clonedf, file.path(path.to.05.output, sprintf("clones_%s.xlsx", mouse.id)))
    } else {
      clonedf <-  readxl::read_excel(file.path(path.to.05.output, sprintf("clones_%s.xlsx", mouse.id)))
    }
    ...
    for (input.VJ.combi in unique(clonesets[[sprintf("VJcombi_CDR3_%s", thres)]])){
      ...
      path.to.output.fasta <- file.path(path.to.05.output, sprintf("%s_%s_%s.aln.fasta", 
                                                                             mouse.id, sprintf("%s_YFP", input.yfp.case), 
